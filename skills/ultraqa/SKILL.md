...
|------------|-----------|---------------|
| `/omc:ultraqa --tests` | tests | All test suites pass |
| `/omc:ultraqa --build` | build | Build succeeds with exit 0 |
| `/omc:ultraqa --lint` | lint | No lint errors |
| `/omc:ultraqa --typecheck` | typecheck | No TypeScript errors |
| `/omc:ultraqa --custom "pattern"` | custom | Custom success pattern in output |
...
1. **RUN QA**: Execute verification based on goal type
   ...
   - `--interactive`: Use qa-tester for interactive CLI/service testing:
     ```
     Task(subagent_type="omc:qa-tester", model="sonnet", prompt="TEST:
...
3. **ARCHITECT DIAGNOSIS**: Spawn architect to analyze failure
   ```
   Task(subagent_type="omc:architect", model="opus", prompt="DIAGNOSE FAILURE:
   Goal: [goal type]
...
4. **FIX ISSUES**: Apply architect's recommendations
   ```
   Task(subagent_type="omc:executor", model="sonnet", prompt="FIX:
...
User can cancel with `/omc:cancel` which clears the state file.
