 ...
 * Ralph Hook
 *
 * Self-referential work loop that continues until cancelled via /omc:cancel.
...
export function createRalphLoopHook(directory: string): RalphLoopHook {
  const startLoop = (
    ...
    if (isUltraQAActive(directory, sessionId)) {
      console.error(
        "Cannot start Ralph Loop while UltraQA is active. Cancel UltraQA first with /omc:cancel.",
      );
      return false;
    }
